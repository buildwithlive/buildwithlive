...
import CheckoutModal from './CheckoutModal'; // Modal එක import කරගත්තා
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 grid grid-cols-1 lg:grid-cols-2 gap-12 items-center relative z-10">
        ...
        >
          <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-blue-900/30 border border-blue-500/30 mb-6">
            ...
            <span className="text-blue-400 text-xs font-bold tracking-wider uppercase">World&apos;s First Live Video Book</span>
          </div>
          ...
          <div className="flex flex-col sm:flex-row items-center gap-4 justify-center lg:justify-start">
            ...
            <div className="flex items-center gap-2 text-gray-400 text-sm">
              <div className="flex -space-x-2">
                {[1,2,3,4].map((i) => (
                  <div key={i} className="w-8 h-8 rounded-full bg-gray-700 border-2 border-black flex items-center justify-center text-xs text-white">U{i}</div>
                ))}
              </div>
        ...
        >
          ...
          <div className="relative w-[300px] md:w-[400px] aspect-[2/3] rounded-r-2xl shadow-2xl shadow-blue-900/50 transform transition-transform hover:scale-105 duration-500 group">
             <div className="absolute inset-0 bg-gray-800 rounded-r-2xl overflow-hidden border-l-8 border-gray-900">
                {/* Make sure sample.jpg is in your public folder */}
                <Image 
                  src="/sample.jpg" 
             ...
             {/* Floating Badge */}
             <div className="absolute -bottom-6 -right-6 bg-black border border-gray-700 p-4 rounded-xl shadow-xl flex items-center gap-3">
                <div className="bg-red-600 p-2 rounded-full">
                    <Play size={20} className="text-white fill-current" />
                </div>
                <div>
                    <p className="text-white font-bold text-sm">250+ Videos</p>
                    <p className="text-gray-400 text-xs">Scan & Watch</p>
                </div>
             </div>
          </div>
        </motion.div>
